...
from mavencode.core import config
from mavencode.core import tasks
from mavencode.core.logger import logger
from mavencode.api.routes.health_route import router as health_router
from mavencode.api.routes.blog.routes import router as blog_router
from mavencode.api.routes.users.routes import router as user_router

BASE_PATH = "/v1/mvc"

tags_metadata = [
    {"name": "Health", "description": "Health status of the API Endpoints"},
    {"name": "MVC", "description": "MavenCode API Routes"},
]
...
def get_application():
    ...
    fast_api.include_router(user_router, prefix=BASE_PATH)
    fast_api.include_router(blog_router, prefix=BASE_PATH)
...
def main():
    logger.info("Starting MavenCode Application API Platform...")
    uvicorn.run(
